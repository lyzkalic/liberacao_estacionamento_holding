...
class WpsIntegrationService:
    ...
    def enviar_liberacao(self, payload, codigo_seguranca, correlation_id):
        ...
        base_wps_url = (
            Config.WPS_URL
            or "https://parebem.parkingplus.com.br/servicos/2/pagamento"
        )
        url = f"{base_wps_url}?apiKey={api_key}&apiKeyId={api_key_id}"
        ...
        try:
            ...
            self.log_transacao(
                correlation_id, "Timeout na comunicação com a WPS"
            )
            return WpsResponse(
                sucesso=False,
                status_http=None,
            ...
            return WpsResponse(
                ...
                resposta={"detalhe_tecnico": str(erro)},
            )
        ...
        self.log_transacao(correlation_id, "Body WPS: %s", resposta.text)
